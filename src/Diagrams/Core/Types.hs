...
data QDiaLeaf v m
  = PrimLeaf (Prim v)
  | DelayedLeaf (DownAnnots v -> Scalar v -> Scalar v -> QDiagram v m)
...
withQDiaLeaf :: (Prim v -> r)
            -> ((DownAnnots v -> Scalar v -> Scalar v -> QDiagram v m) -> r)
            -> (QDiaLeaf v m -> r)
...
applyAnnotation
  ...
  => Annotation -> QDiagram v m -> QDiagram v m
...
href :: (HasLinearMap v, InnerSpace v, OrderedField (Scalar v), Semigroup m) => String -> QDiagram v m -> QDiagram v m
href = applyAnnotation . Href

-- | The fundamental diagram type is represeted by trees of
...
--   specialized to 'Any'.
newtype QDiagram v m
  = QD (D.DUALTree (DownAnnots v) (UpAnnots v m) Annotation (QDiaLeaf v m))
  deriving (Typeable)

instance Wrapped (QDiagram v m) where
    type Unwrapped (QDiagram v m) =
        D.DUALTree (DownAnnots v) (UpAnnots v m) Annotation (QDiaLeaf v m)
...
envelope :: forall v m. (OrderedField (Scalar v), InnerSpace v
                          , HasLinearMap v, Monoid' m)
         => Lens' (QDiagram v m) (Envelope v)
envelope = lens (unDelete . getU' . view _Wrapped') (flip setEnvelope)

-- | Replace the envelope of a diagram.
setEnvelope :: forall v m. (OrderedField (Scalar v), InnerSpace v
                             , HasLinearMap v, Monoid' m)
          => Envelope v -> QDiagram v m -> QDiagram v m
setEnvelope e =
...
trace :: (InnerSpace v, HasLinearMap v, OrderedField (Scalar v), Semigroup m) =>
         Lens' (QDiagram v m) (Trace v)
trace = lens (unDelete . getU' . view _Wrapped') (flip setTrace)

-- | Replace the trace of a diagram.
setTrace :: forall v m. (OrderedField (Scalar v), InnerSpace v
                          , HasLinearMap v, Semigroup m)
         => Trace v -> QDiagram v m -> QDiagram v m
...
subMap = lens (unDelete . getU' . view _Wrapped') (flip setMap) where
  setMap :: (HasLinearMap v, InnerSpace v, Semigroup m, OrderedField (Scalar v)) =>
            SubMap v m -> QDiagram v m -> QDiagram v m
...
nameSub :: ( IsName n
           , HasLinearMap v, InnerSpace v, OrderedField (Scalar v), Semigroup m)
        => (QDiagram v m -> Subdiagram v m) -> n -> QDiagram v m -> QDiagram v m
nameSub s n d = d'
...
lookupName :: (IsName n, HasLinearMap v, InnerSpace v
           ...
           => n -> QDiagram v m -> Maybe (Subdiagram v m)
...
withName :: (IsName n, HasLinearMap v, InnerSpace v
            , Semigroup m, OrderedField (Scalar v))
         => n -> (Subdiagram v m -> QDiagram v m -> QDiagram v m)
...
localize :: forall v m. ( HasLinearMap v, InnerSpace v
                          , OrderedField (Scalar v), Semigroup m
                          )
         => QDiagram v m -> QDiagram v m
localize = over _Wrapped' ( D.applyUpre  (inj (deleteL :: Deletable (SubMap v m)))
                   . D.applyUpost (inj (deleteR :: Deletable (SubMap v m)))
                   )


-- | Get the query function associated with a diagram.
query :: Monoid m => QDiagram v m -> Query v m
query = getU' . view _Wrapped'

-- | Sample a diagram's query function at a given point.
sample :: Monoid m => QDiagram v m -> Point v -> m
...
value :: Monoid m => m -> QDiagram v Any -> QDiagram v m
...
mkQD :: Prim v -> Envelope v -> Trace v -> SubMap v m -> Query v m
     -> QDiagram v m
mkQD p = mkQD' (PrimLeaf p)

-- | Create a diagram from a generic QDiaLeaf, along with an envelope,
--   trace, subdiagram map, and query function.
mkQD' :: QDiaLeaf v m -> Envelope v -> Trace v -> SubMap v m -> Query v m
      -> QDiagram v m
mkQD' l e t n q
...
instance (HasLinearMap v, InnerSpace v, OrderedField (Scalar v), Semigroup m)
  => Monoid (QDiagram v m) where
  mempty  = QD D.empty
  mappend = (<>)

instance (HasLinearMap v, InnerSpace v, OrderedField (Scalar v), Semigroup m)
  => Semigroup (QDiagram v m) where
...
instance (HasLinearMap v, OrderedField (Scalar v), InnerSpace v, Semigroup m)
      => Transformable (QDiagram v m) where
...
instance (HasLinearMap v, InnerSpace v, OrderedField (Scalar v), Semigroup m)
      => Qualifiable (QDiagram v m) where
...
data Subdiagram v m = Subdiagram (QDiagram v m) (DownAnnots v)
...
--   \"ended up\".
location :: HasLinearMap v => Subdiagram v m -> Point v
...
newtype SubMap v m = SubMap (M.Map Name [Subdiagram v m])
  -- See Note [SubMap Set vs list]

instance Wrapped (SubMap v m) where
    type Unwrapped (SubMap v m) = M.Map Name [Subdiagram v m]
    _Wrapped' = iso (\(SubMap m) -> m) SubMap

instance Rewrapped (SubMap v m) (SubMap v' m')
...
instance Functor (SubMap v) where
  fmap = over _Wrapped . fmap . map . fmap

instance Semigroup (SubMap v m) where
...
--   @a@.
instance Qualifiable (SubMap v m) where
  a |> (SubMap m) = SubMap $ M.mapKeys (a |>) m

-- | Construct a 'SubMap' from a list of associations between names
--   and subdiagrams.
fromNames :: IsName a => [(a, Subdiagram v m)] -> SubMap v m
fromNames = SubMap . M.fromListWith (++) . map (toName *** (:[]))

-- | Add a name/diagram association to a submap.
rememberAs :: IsName a => a -> QDiagram v m -> SubMap v m -> SubMap v m
rememberAs n d = over _Wrapped' $ M.insertWith (++) (toName n) [mkSubdiagram d]

-- | A name acts on a name map by qualifying every name in it.
instance Action Name (SubMap v m) where
  act = (|>)
...
lookupSub :: IsName n => n -> SubMap v m -> Maybe [Subdiagram v m]
...
-- | A value of type @Prim v@ is an opaque (existentially quantified)
--   primitive which backend @b@ knows how to render in vector space @v@.
data Prim v where
  Prim :: (Transformable p, Typeable p) => p -> Prim (V p)

type instance V (Prim v) = v

-- | The 'Transformable' instance for 'Prim' just pushes calls to
--   'transform' down through the 'Prim' constructor.
instance HasLinearMap v => Transformable (Prim v) where
...
type DTree v a = Tree (DNode v a)

data RNode v a =  RStyle (Style v)
                    -- ^ A style node.
                  | RAnnot a
                  | RPrim (Prim v)
                    -- ^ A primitive.
...
class HasLinearMap v => Backend b v where
  ...
  adjustDia :: (Monoid' m, Num (Scalar v)) => b -> Options b v
            -> QDiagram v m -> (Options b v, Transformation v, QDiagram v m)
  ...
  renderRTree :: b -> Options b v -> RTree v Annotation -> Result b v
